...
# 팀과제 목표 : 
# 이 과제를 수행하였을 때 변수의 독립과 종속 관계를
# 파악할수 있고 나이브 베이지안, 베이지안 분류모델을 이해하고
# 앞으로의 데이터 분류에 이용 할 수 있게 된다
...
# 카이제곱 검정 : 독립성 검정 => 아이리스 데이터는 
#수치형 예측변수 이기에 카이제곱 불가
chisq.test(data[,-5])
...
#sepal.length와 sepal.width
tt <- cbind(training$Sepal.Length,training$Sepal.Width,training$Species)
tt
...
# 독립이 아닌 petal.width값 배제
# 일반 모델 vs 나이브 베이지안 모델 비교 ==> 0.9166vs1
indxTrain <- createDataPartition(y = data$Species,p = 0.75,list = FALSE)
indxTrain
training <- data[indxTrain,] # 트레이닝
training
...
#분류 정확도
(t[1,1]+t[2,2]+t[3,3])/nrow(testing)
confusionMatrix(p, testing$Species) # 1
#-------------------------------------------------------------
fit <- rpart(Species~.,
             data = x,
             minsplit = 2,
...
#sepal.length, petal.length 독립성이 큰 예측 변수 쌍
